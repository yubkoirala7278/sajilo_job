                ...
                <div class="col-12">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <ul class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item">
                                <a href="{{ route('employee_course.index') }}" style="color: #2C3E50">Employee Course</a>
                            </li>
                        </ul>
                        <a href="{{ route('employee_course.create') }}" class="btn text-white btn-sm rounded-pill px-3 py-2"
                            style="background-color: #2C3E50">Create</a>
                    </div>
                    <div class="table-responsive">
                        <table class="table course-datatable table-hover pt-3 w-100">
                            <thead>
                                <tr>
                                    <th>S.N:</th>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var table = $('.course-datatable').DataTable({
                ...
                serverSide: true,
                searchDelay: 1000,
                ajax: "{{ route('employee_course.index') }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
                        orderable: false,
            ...
            $(document).on('click', '.toggle-status-btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `/admin/employee_course/toggle-status/${slug}`,
                            type: 'PATCH',
                            data: {
                                status: newStatus,
                                _token: '{{ csrf_token() }}',
            ...
            $(document).on('click', '.delete-btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: '/admin/employee_course/' + slug,
                            type: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
                            success: function(response) {
                                if (response.status === 'success') {
                                    Swal.fire(
                                        'Deleted!',
                                        'Employee course has been deleted.',
                                        'success'
                                    );
                                    table.ajax
