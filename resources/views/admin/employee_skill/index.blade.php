                ...
                <div class="col-12">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <ul class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item">
                                <a href="{{ route('employee_skill.index') }}" style="color: #2C3E50">Employee Skill</a>
                            </li>
                        </ul>
                        <a href="{{ route('employee_skill.create') }}" class="btn text-white btn-sm rounded-pill px-3 py-2"
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var table = $('.skill-datatable').DataTable({
                ...
                serverSide: true,
                searchDelay: 1000,
                ajax: "{{ route('employee_skill.index') }}",
                columns: [
                    {
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
            ...
            $(document).on('click', '.toggle-status-btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `/admin/employee_skill/toggle-status/${slug}`,
                            type: 'PATCH',
                            data: {
                                status: newStatus,
                                _token: '{{ csrf_token() }}',
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.status === 'success') {
                                    Swal.fire(
                                        'Deleted!',
                                        'Employee skill has been deleted.',
                                        'success'
                                    );
                                    table.ajax
