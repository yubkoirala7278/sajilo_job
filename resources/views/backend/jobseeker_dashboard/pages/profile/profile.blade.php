    ...
    <div class="p-4">
        ...
        <div class="mb-4">
            <h1 class="fs-4 fw-bold text-primary">
                <i class="fas fa-user-circle me-2"></i> Hello, {{ Auth::user()->name }}
            </h1>
            <p class="text-muted">Here is your complete profile overview</p>
        </div>

        <!-- Profile Card -->
        <div class="card shadow-sm border-0">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="fas fa-info-circle me-2"></i> Profile Details</h5>
            </div>
            <div class="card-body">
                <!-- Basic Information -->
                <section class="mb-5">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            @if($employee->resume)
                                <a href="{{ Storage::url($employee->resume) }}" target="_blank" class="text-primary">View Resume</a>
                            @else
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            @if($employee->profile)
                                <a href="{{ Storage::url($employee->profile) }}" target="_blank"><img src="{{ Storage::url($employee->profile) }}" alt="Profile" class="img-thumbnail" style="max-width: 100px;"></a>
                            @else
                                <span>Not uploaded</span>
                            @endif
                        </div>
                    ...
                    @if($employee->trainings->isNotEmpty())
                        @foreach($employee->trainings as $training)
                            <div class="card mb-3 border-light shadow-sm">
                                <div class="card-body">
                                    <p><strong><i class="fas fa-chalkboard-teacher me-1"></i> Training Name:</strong> {{ $training->training_name }}</p>
                                    <p><strong><i class="fas fa-building me-1"></i> Institution:</strong> {{ $training->institution_name }}</p>
                                    <p><strong><i class="fas fa-clock me-1"></i> Duration:</strong> {{ $training->training_duration_operator }} {{ $training->training_duration }}</p>
                                    <p><strong><i class="fas fa-calendar-alt me-1"></i> Completion Date:</strong> {{ $training->training_completion_date ?? 'Not specified' }}</p>
                                </div>
                            </div>
                        @endforeach
                    @else
                        <p class="text-muted"><i class="fas fa-exclamation-circle me-1"></i> No training records available.</p>
                    @endif
                </section>
                ...
                <section class="mb-5">
                    ...
                    @if($employee->experiences->isNotEmpty())
                        @foreach($employee->experiences as $experience)
                            <div class="card mb-3 border-light shadow-sm">
                                <div class="card-body">
                                    <p><strong><i class="fas fa-building me-1"></i> Organization:</strong> {{ $experience->organization_name }}</p>
                                    <p><strong><i class="fas fa-map-marker-alt me-1"></i> Location:</strong> {{ $experience->job_location }}</p>
                                    <p><strong><i class="fas fa-tag me-1"></i> Job Title:</strong> {{ $experience->job_title }}</p>
                                    <p><strong><i class="fas fa-level-up-alt me-1"></i> Job Level:</strong> {{ $experience->job_level }}</p>
                                    ...
                                    <p><strong><i class="fas fa-tasks me-1"></i> Duties:</strong> {{ $experience->duties_and_responsibilities ?? 'Not specified' }}</p>
                                    <p><strong><i class="fas fa-industry me-1"></i> Organization Nature:</strong> {{ optional($experience->organizationNature)->name ?? 'Not specified' }}</p>
                                    <p><strong><i class="fas fa-folder-open me-1"></i> Job Category:</strong> {{ optional($experience->jobCategory)->category ?? 'Not specified' }}</p>
                                </div>
                            </div>
                        @endforeach
                    @else
                        <p class="text-muted"><i class="fas fa-exclamation-circle me-1"></i> No experience records available.</p>
                    @endif
                </section>
                            ...
                            @foreach($employee->languages as $language)
                                <div class="col-md-6 mb-3">
                                    <div class="card border-light shadow-sm">
                                        <div class="card-body">
                                            <p><strong><i class="fas fa-comment me-1"></i> Language:</strong> {{ $language->language }}</p>
                                            <p><strong><i class="fas fa-book me-1"></i> Reading:</strong> 
                                                @switch($language->reading)
                                                    @case('very_good')
                                                        ...
                                                        @break
                                                    @case('good')
                                                        <span class="text-warning"><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="far fa-star"></i><i class="far fa-star"></i></span>
                                                        @break
                                                    @case('average')
                                                        <span class="text-warning"><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="far fa-star"></i><i class="far fa-star"></i><i class="far fa-star"></i></span>
                                                        @break
                                                    @case('poor')
                                                        <span class="text-warning"><i class="fas fa-star"></i><i class="far fa-star"></i><i class="far fa-star"></i><i class="far fa-star"></i><i class="far fa-star"></i></span>
                                                        @break
                                                    @case('very_poor')
                                                        <span class="text-warning"><i class="far fa-star"></i><i class="far fa-star"></i><i class="far fa-star"></i><i class="far fa-star"></i><i class="far fa-star"></i></span>
                                                        @break
                                                    @default
                    ...
                    @if($employee->socialAccounts->isNotEmpty())
                        @foreach($employee->socialAccounts as $account)
                            <p>
                                ...
                                <a href="{{ $account->account_url }}" target="_blank" class="text-primary">{{ $account->account_url }}</a>
                            </p>
                        @endforeach
                    @else
                        <p class="text-muted"><i class="fas fa-exclamation-circle me-1"></i> No social accounts linked.</p>
                    @endif
                </section>
